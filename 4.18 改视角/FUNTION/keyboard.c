...
void kb_adrust_angle_ctrl(void)//机械臂角度控制
{
	...
	chassis_mode == CHASSIS_EXCHANGE_MODE))
	{
		...
		if(KB_YAW_LEFT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[ADRUST_YAW],+1,1);
		}
		if(KB_YAW_RIGHT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[ADRUST_YAW],-1,1);
		...
		if(KB_PITCH_UP)
		{
			kb_kb_adrust_angle_change(&kb_adrust[ADRUST_PITCH],1,10);
		}
		if(KB_PITCH_DN)
		{
			kb_kb_adrust_angle_change(&kb_adrust[ADRUST_PITCH],-1,10);
		...
		if(KB_ROLL_LEFT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[ADRUST_ROLL],5,10);
		}
		if(KB_ROLL_RIGHT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[ADRUST_ROLL],-5,10);
