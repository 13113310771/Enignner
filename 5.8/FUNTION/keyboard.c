...
void kb_adrust_angle_ctrl(void) // 机械臂角度控制
{
	...
	{
		if (KB_P_T_PITCH_RIGHT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[P_T_PITCH], 0.5, 1);
		}
		if (KB_P_T_PITCH_LEFT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[P_T_PITCH], -0.5, 1);
		}
		...
		if (KB_P_T_YAW_RIGHT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[P_T_YAW], -0.5, 1);
		}
		if (KB_P_T_YAW_LEFT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[P_T_YAW], 0.5, 1);
		...
		if (KB_JOINT1_RIGHT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[JOINT_1], -0.01, 1);
		}
		if (KB_JOINT1_LEFT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[JOINT_1], 0.01, 1);
		...
		if (KB_JOINT2_RIGHT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[JOINT_2], -0.01, 1);
		}
		if (KB_JOINT2_LEFT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[JOINT_2], 0.01, 1);
		...
		if (KB_UPRAISE_UP_BIG)
		{
			kb_kb_adrust_angle_change(&kb_adrust[UPRISE], 4, 1);
		}
		if (KB_UPRAISE_DN_BIG)
		{
			kb_kb_adrust_angle_change(&kb_adrust[UPRISE], -4, 1);
		}
		if (ANCTOR_PITCH_UP)
		{
			kb_kb_adrust_angle_change(&kb_adrust[ADRUST_PITCH], 0.4, 1);
		}if (ANCTOR_PITCH_DN)
		{
			kb_kb_adrust_angle_change(&kb_adrust[ADRUST_PITCH], -0.4, 1);
		}if (ANCTOR_ROLL_LEFT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[ADRUST_ROLL], -0.4, 1);
		}if (ANCTOR_ROLL_RIGHT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[ADRUST_ROLL], 0.4, 1);
		}
		if (KB_YAW_LEFT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[ADRUST_YAW], 0.5, 1);
		}if (KB_YAW_RIGHT)
		{
			kb_kb_adrust_angle_change(&kb_adrust[ADRUST_YAW], -0.5, 1);
		}
