...
void big_island_stright_clamp_handler(void)
{
	switch(clamp_action)
	{
		case CLAMP_UN_CMD:
		{
			Motor_change_mode_angle(&Mode.bigisland_straight_angle,GIMBAL_YAW,MOTOR_MAX_ID);//只将夹取相关电机置对应模式角度
...
void Motor_uprise_clamp_angle_change(int16_t change_displacement)
{
