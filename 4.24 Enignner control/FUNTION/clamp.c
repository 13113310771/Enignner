...
void clamp_angle_handle()
{
	...
	Motor[UPRAISE].Brushless.angle_ref +=  model_out_angle[0];
...
void big_island_slanted_clamp_handler(void)
{
	...
	case CLAMP_UN_CMD:
	{
		for (int i = 0; i <= keyboard_number; i++)
		{
			kb_adrust[i].angle = 0;
		}
...
void small_island_clamp_handler(void)
{
	...
	case ACTION_ONE:
	{
		if (action_one_state == ACTIONING)
		{
			...
			{
				Motor_uprise_clamp_angle_change(23);
			}
...
uint8_t store_handler(void)
{
	...
	if (chassis_mode == CHASSIS_CLAMP_BIG_ISLAND_SLANTED_MODE || chassis_mode == CHASSIS_CLAMP_BIG_ISLAND_STRAIGHT_MODE)
	{
		...
		{		
			Motor[joint_1].Angle.mode_angle = -1.239 * 6.33;
			Motor[joint_2].Angle.mode_angle = 0.535 * 6.33;
		}
...
uint8_t pick_handler(void)
{
	...
	{
		Motor[CLAMP_SILD].Angle.mode_angle = 10*45;
		Motor[UPRAISE_CLAMP].Angle.mode_angle = -35*22;
		Motor[UPRAISE].Angle.mode_angle = 700;
