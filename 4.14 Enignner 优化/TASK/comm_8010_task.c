	...
	while (1)
	{
		...
		if (STAUS == pdTRUE)
		{
			...
			{				
					...
					go_8010_test_tesk1(GO_8010_1,0,tamp_task(receive_angle[0],tete,target_angle[0]),0,1,0.05);
					vTaskDelayUntil(&comm_8010_time, 8);
					go_8010_test_tesk1(GO_8010_2,0,tamp_task(receive_angle[1],tete,target_angle[1]),0,1,0.05);
					vTaskDelayUntil(&comm_8010_time, 8);
			...
			if(Signal == GO_8010_INIT_SIGNAL)
		{
			...
			{
				 go_8010_init_task(GO_8010_1,0.2);
			}	
